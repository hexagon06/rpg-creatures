<template>
  <div class="flex gap-2 flex-col">
    <div v-if="weighted"
         class="ml-20">{{ total }}</div>
    <div class="flex flex-col gap-2">
      <div v-for="(item, i) in orderedList"
           :key="i"
           class="flex-grow grid grid-flow-row-dense list-grid-template gap-2">
        <span class="flex gap-2">
          <button @click="moveItem(item.order, 'up')"
                  class="button-round button-on-rouge"
                  title="move up"
                  :disabled="item.order === 0">
            <font-awesome-icon icon="fa-solid fa-angle-up" />
          </button>
          <button @click="moveItem(item.order, 'down')"
                  class="button-round button-on-rouge"
                  title="move down"
        ...
        <span class="col-start-2 col-span-3 lg:col-span-1 flex gap-2">
          <input v-model="item.label"
                 ...
                 @blur="updateModel" />
          <button v-if="item.reference === undefined || isLinking(item)"
                  @click="linking(item)"
                  class="button-round break-before-auto"
                  :class="isLinking(item) ? 'button-on-brown' : 'button-on-rouge'">
            <font-awesome-icon v-if="!isLinking(item)"
                               icon="fa-solid fa-link" />
            <font-awesome-icon v-if="isLinking(item)"
                               icon="fa-solid fa-link-slash" />
          </button>

          <input v-if="isLinking(item)"
                 v-model="item.reference"
                 class="w-0 flex-grow flex-initial"
                 @blur="updateModel" />

          <reference-picker v-if="item.reference === undefined || isReferencing(item)"
                            :reference.sync="item.reference" />
        </span>

        <span class="col-end-5 flex gap-2">
          <button @click="repeat(item)"
                  class="button-round"
    ...
    <button @click="create()"
            class="button-round-large button-on-rouge">
...
export default defineComponent({
  ...
  computed: {
    orderedList(): ItemType[] {
      return sortBy(this.modelValue, (v) => v.order);
    },
    total(): number {
      return sum(this.modelValue.map((v) => Number(v.weight)));
    },
  },
  methods: {
    ...
    create() {
      ...
      this.$emit("update:value", cloneDeep(this.modelValue).concat([item]));
    },
    remove(index: number) {
      const item = this.orderedList[index];
      this.$emit(
        "update:value",
        this.modelValue.filter((v) => v.id !== item.id)
      );
    },
    updateModel() {
      this.$emit("update:value", cloneDeep(this.modelValue));
    },
    ...
    moveItem(order: number, direction: "up" | "down") {
      if ((order > 0 || order < this.orderedList.length - 1) && this.modelValue) {
        if (direction === "up") {
          const before = this.modelValue.find((s) => s.order === order - 1);
          const target = this.modelValue.find((s) => s.order === order);
          before!.order++;
          target!.order--;
...
.list-grid-template {
  grid-template-columns: fit-content(2rem) fit-content(4rem) 1fr fit-content(2rem);
}
</style>

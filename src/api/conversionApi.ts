...
export class Converter<TOld extends IdItem, TNew extends IdItem> {
  ...
  constructor(private oldApi: ApiGetList<TOld>, private newApi: ApiCreate<TNew>, private converter: (old: TOld) => TNew) { }
  ...
  async start() {
    ...
    for (const { id } of indexes) {
      ...
      if (old === undefined) throw Error(`could not get old creature with id '${id}'`)
